...
def show_production_page(df):
    ...
    show_avg = st.sidebar.checkbox("Show average of all countries", value=True)
...
def show_trade_throughput_page(df):
    ...
    # Year slider
    min_comp_year = int(df["year"].min())
    max_comp_year = int(df["year"].max())
    ...
    metrics = ["exports", "imports", "consumption", "production"]

    df_filtered = df[
        (df["year"] == selected_comp_year) &
        (df["country"].isin(selected_comp_countries))
        ].copy()
...
if __name__ == "__main__":
    ...
    page_selection = st.sidebar.radio("Go to", ["Production Trends", "Trade and Throughput market Comparison"])
